...
$a->addDriver(new \Doctrine\ORM\Mapping\Driver\AnnotationDriver(($this->privates['annotations.cached_reader'] ?? $this->load('getAnnotations_CachedReaderService.php')), array(0 => ($this->targetDirs[3].'/src/Domain/Model/Entity'))), 'App\\Domain\\Model\\Entity');

$b = new \Doctrine\ORM\Configuration();
$b->setEntityNamespaces(array('App' => 'App\\Domain\\Model\\Entity'));
...
$b->setRepositoryFactory(new \Doctrine\Bundle\DoctrineBundle\Repository\ContainerRepositoryFactory(new \Symfony\Component\DependencyInjection\ServiceLocator(array('App\\Infrastructure\\Repository\\DepartmentRepository\\DepartmentRepository' => function () {
    return ($this->privates['App\Infrastructure\Repository\DepartmentRepository\DepartmentRepository'] ?? $this->load('getDepartmentRepositoryService.php'));
}, 'App\\Infrastructure\\Repository\\SubDepartmentRepository\\SubDepartmentRepository' => function () {
    return ($this->privates['App\Infrastructure\Repository\SubDepartmentRepository\SubDepartmentRepository'] ?? $this->load('getSubDepartmentRepositoryService.php'));
}, 'App\\Infrastructure\\Repository\\WorkerRepository\\WorkerRepository' => function () {
    return ($this->privates['App\Infrastructure\Repository\WorkerRepository\WorkerRepository'] ?? $this->load('getWorkerRepositoryService.php'));
}, 'App\\Infrastructure\\Repository\\WorkerStatusRepository\\WorkerStatusRepository' => function () {
    return ($this->privates['App\Infrastructure\Repository\WorkerStatusRepository\WorkerStatusRepository'] ?? $this->load('getWorkerStatusRepositoryService.php'));
}))));
